...
int main() {
  ...
  const uint RIGHT_WHEEL_PWM_PIN = 5;
  const uint RIGHT_WHEEL_DIR_PIN = 4;

  Encoder leftEncoder(LEFT_ENCODER_PIN);
  Encoder rightEncoder(RIGHT_ENCODER_PIN);
  Wheel wheel1(LEFT_WHEEL_PWM_PIN, LEFT_WHEEL_DIR_PIN);
  Wheel wheel2(RIGHT_WHEEL_PWM_PIN, RIGHT_WHEEL_DIR_PIN);
  ...
  Movements movements(wheel1, wheel2, leftEncoder, rightEncoder, pid);
